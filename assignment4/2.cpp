...
struct CircularQueue {
    int n, front, rear, cnt;
    vector<int> a;
    CircularQueue(int cap): n(cap), front(0), rear(-1), cnt(0), a(cap) {}
    bool isEmpty() const { return cnt==0; }
    bool isFull()  const { return cnt==n; }
    bool enqueue(int x){ if(isFull()) return false; rear = (rear + 1) % n; a[rear] = x; cnt++; return true; }
    bool dequeue(int &out){ if(isEmpty()) return false; out = a[front]; front = (front + 1) % n; cnt--; return true; }
    bool peek(int &out) const { if(isEmpty()) return false; out = a[front]; return true; }
    void display() const {
        if(isEmpty()){ cout<<"Queue is empty\n"; return; }
        for(int i=0;i<cnt;i++) cout << a[(front+i)%n] << (i+1<cnt?' ':'\n');
    }
};
